...
public final class LinkedList<T> {
    ...
    public class MsgNode<T>: Identifiable, ObservableObject {
        var id: T { return value }
        var value: T
        var next: MsgNode?
        weak var previous: MsgNode?
    ...
    public typealias Node = MsgNode<T>
...
public struct LinkedListIndex<T>: Comparable {
    fileprivate let node: LinkedList<T>.MsgNode<T>?
...
extension LinkedList: RandomAccessCollection {
    ...
    public func index(before i: LinkedListIndex<T>) -> LinkedListIndex<T> {
        return LinkedListIndex<T>(node: i.node?.previous, tag: i.tag - 1)
    }
